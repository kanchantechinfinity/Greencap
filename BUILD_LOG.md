...
## 2026-08-17 — Flip cards reverted, kept the one-row layout (`193e1ee`)

User: "ITS LOOKING UGLY KEEP THAT SECTION LIKE BEFORE ONLY JUST 5 ONE PLACE IN MIDDLE" — the flip-card redesign didn't land well even after the size fix. Restored the original icon+text markup verbatim from commit `0917804` (the version right after the one-row grid fix, right before flip cards were introduced) via `git show 0917804:index.html`, so the content is byte-for-byte what it was — sage-tint rounded box, icon left, label+description stacked right — while keeping the `.features-banner` grid (2/3/5 responsive columns) so the "all 5 in one row, centered" fix from that same earlier commit stays in place.
